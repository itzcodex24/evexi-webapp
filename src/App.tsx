...
import { Evexi, EvexiMock } from "./evexi.modern.es";
...
function App() {
  ...
  useEffect(() => {
    ...
    async function getVars() {
      ...
      console.log(LOGO, COLORS, API);

      setVars({ LOGO, COLORS, API });
    }
    getVars();
  }, []);
  ...
  useEffect(() => {
    document.documentElement.style.setProperty(
      "--primary-color",
      vars?.COLORS.PRIMARY_COLOR ?? "#000000"
    );
    document.documentElement.style.setProperty(
      "--secondary-color",
      vars?.COLORS.SECONDARY_COLOR ?? "#000000"
    );
    document.documentElement.style.setProperty(
      "--tertiary-color",
      vars?.COLORS.TERTIARY_COLOR ?? "#000000"
    );

    async function getEvents() {
      const { API_KEY, CID } = vars?.API;
